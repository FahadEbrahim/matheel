...
:root {
  ...
  --panel-strong: rgba(255, 251, 242, 0.94);
  --line: rgba(17, 32, 29, 0.1);
  --accent: #d25e2b;
  --accent-2: #187a65;
  --shadow: 0 24px 60px rgba(17, 32, 29, 0.10);
}

.gradio-container {
  background:
    radial-gradient(circle at top left, rgba(210, 94, 43, 0.18), transparent 34%),
    radial-gradient(circle at top right, rgba(24, 122, 101, 0.16), transparent 30%),
...
.hero-banner {
  background: linear-gradient(135deg, rgba(255, 248, 236, 0.96), rgba(246, 241, 229, 0.92));
  border: 1px solid rgba(17, 32, 29, 0.08);
  border-radius: 28px;
...
.hero-panel {
  background: rgba(17, 32, 29, 0.05);
  border: 1px solid rgba(17, 32, 29, 0.06);
  border-radius: 22px;
  padding: 18px;
  align-self: stretch;
}
...
.status-strip {
  border-radius: 16px;
  padding: 12px 14px;
  background: linear-gradient(135deg, rgba(255, 255, 255, 0.88), rgba(247, 239, 227, 0.8));
  border: 1px solid rgba(17, 32, 29, 0.06);
}
...
.score-card {
  border-radius: 20px;
  padding: 14px;
  background: linear-gradient(145deg, rgba(255, 255, 255, 0.9), rgba(247, 239, 227, 0.86));
  border: 1px solid rgba(17, 32, 29, 0.08);
}
...
.summary-card {
  border-radius: 20px;
  padding: 18px;
  background: linear-gradient(145deg, rgba(255, 255, 255, 0.92), rgba(247, 239, 227, 0.88));
  border: 1px solid rgba(17, 32, 29, 0.08);
  margin-bottom: 14px;
}
...
.primary-run button {
  background: linear-gradient(90deg, var(--accent), #ea7b3d) !important;
  border: none !important;
  color: white !important;
}

.secondary-run button {
  background: linear-gradient(90deg, var(--accent-2), #2f967b) !important;
  border: none !important;
  color: white !important;
}
...
<section class="hero-banner">
  <div class="hero-grid">
    <div>
      <span class="hero-kicker">Matheel Studio</span>
      <h1>Shape the pipeline, not just the score.</h1>
      <p>
        Tune preprocessing, chunking, vector backends, and code-aware metrics from one surface.
      </p>
    </div>
    <div class="hero-panel">
      <div class="hero-chip-grid">
        <div class="hero-chip"><strong>Pair Study</strong><span>Inspect one code pair with rich controls.</span></div>
        <div class="hero-chip"><strong>Collection Study</strong><span>Rank an archive and review the strongest pairs.</span></div>
        <div class="hero-chip"><strong>Comparison Suite</strong><span>Run multiple configurations from one JSON sheet.</span></div>
        <div class="hero-chip"><strong>Code-Aware</strong><span>Layer CodeBLEU-style and CrystalBLEU scoring.</span></div>
      </div>
    </div>
  </div>
</section>
...
LANGUAGE_HINT = ", ".join(available_code_metric_languages())
...
def normalize_weights(ws, wl, wj):
    weights = [max(0.0, min(1.0, float(ws))), max(0.0, min(1.0, float(wl))), max(0.0, min(1.0, float(wj)))]
    total = sum(weights)
    if total <= 0:
        return 0.7, 0.3, 0.0
    normalized = [round(value / total, 2) for value in weights]
    normalized[2] = round(max(0.0, 1.0 - normalized[0] - normalized[1]), 2)
    return normalized[0], normalized[1], normalized[2]


def connect_weight_sync(ws_component, wl_component, wj_component):
    ws_component.change(normalize_weights, [ws_component, wl_component, wj_component], [ws_component, wl_component, wj_component])
    wl_component.change(normalize_weights, [ws_component, wl_component, wj_component], [ws_component, wl_component, wj_component])
    wj_component.change(normalize_weights, [ws_component, wl_component, wj_component], [ws_component, wl_component, wj_component])
...
def preset_profile(name):
    profiles = {
        ...
        "code_focus": {
            ...
            "code_metric_weight": 0.20,
            "code_language": "java",
            "codebleu_component_weights": "0.40,0.30,0.20,0.10",
            "crystalbleu_max_order": 4,
            "crystalbleu_trivial_ngram_count": 500,
            "vector_backend": "sentence_transformers",
            "similarity_function": "dot",
        ...
        "multivector_deep": {
            ...
            "codebleu_component_weights": "0.25,0.25,0.25,0.25",
            "crystalbleu_max_order": 4,
            "crystalbleu_trivial_ngram_count": 200,
...
def pair_preset_values(name):
    config = preset_profile(name)
    return (
        config["use_semantic"],
        config["Ws"],
        config["use_levenshtein"],
        config["Wl"],
        config["use_jaro_winkler"],
        config["Wj"],
        ...
        config["max_chunks"],
        config["chunk_aggregation"],
        config["chunk_language"],
        config["chunker_options"],
        config["code_metric"],
        config["code_metric_weight"],
        config["code_language"],
        config["codebleu_component_weights"],
        config["crystalbleu_max_order"],
        config["crystalbleu_trivial_ngram_count"],
        config["vector_backend"],
        config["similarity_function"],
        config["pooling_method"],
        config["max_token_length"],
        config["static_vector_dim"],
        config["runtime_device"],
    )


def collection_preset_values(name):
    config = preset_profile(name)
    return (
        config["use_semantic"],
        config["Ws"],
...
def mode_status_html(mode):
    layouts = {
        "focused": "Focused mode keeps only the core pipeline visible for quick live demos and clean screenshots.",
        "balanced": "Balanced mode exposes vector controls while keeping code-aware scoring available but out of the way.",
        "full": "Full mode unlocks every advanced control, including code metrics, multivector tuning, and runtime selection.",
    }
    normalized = (mode or "balanced").lower()
    copy = layouts.get(normalized, layouts["balanced"])
    return f"""
    <div class="status-strip">
      <strong>{normalized.title()} View</strong>
      <span>{copy}</span>
    </div>
    """
...
def toggle_visibility(is_visible):
    return gr.update(visible=bool(is_visible))


def interface_mode_updates(mode):
    normalized = (mode or "balanced").lower()
    settings = {
        "focused": (False, False),
        "balanced": (False, True),
    ...
    show_code, show_vector = settings.get(normalized, settings["balanced"])
    return (
        gr.update(visible=show_code),
        gr.update(visible=show_vector),
        mode_status_html(normalized),
    )


def normalize_profile_name(name, store, prefix):
    cleaned = (name or "").strip()
    if cleaned:
        return cleaned
    existing = set((store or {}).keys())
    index = len(existing) + 1
    candidate = f"{prefix} Profile {index}"
...
def save_profile(store, name, fields, values, prefix):
    next_store = dict(store or {})
    profile_name = normalize_profile_name(name, next_store, prefix)
    next_store[profile_name] = dict(zip(fields, values))
    return (
        next_store,
        gr.update(choices=sorted(next_store.keys()), value=profile_name),
        profile_status_html(f'Saved "{profile_name}" with {len(fields)} settings for this session.'),
    )


def load_profile(name, store, fields):
    config = dict((store or {}).get(name) or {})
    if not config:
        default_profile = preset_profile("balanced")
        config = {field: default_profile[field] for field in fields}
    return tuple(config.get(field) for field in fields)


def save_pair_profile(store, name, *values):
    return save_profile(store, name, PAIR_PROFILE_FIELDS, values, "Pair")


def load_pair_profile(name, store):
    return load_profile(name, store, PAIR_PROFILE_FIELDS)


def save_collection_profile(store, name, *values):
    return save_profile(store, name, COLLECTION_PROFILE_FIELDS, values, "Collection")


def load_collection_profile(name, store):
    return load_profile(name, store, COLLECTION_PROFILE_FIELDS)
...
def default_suite_rows():
    return pd.DataFrame(
        [
            ...
            {
                "run_name": "crystalbleu_multivector",
...
def export_suite_runs_gradio(run_sheet_rows):
    run_configs = suite_rows_to_configs(run_sheet_rows)
    export_root = tempfile.mkdtemp(prefix="matheel-suite-runs-")
    export_path = os.path.join(export_root, "comparison_suite_runs.json")
